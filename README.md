...
Use Claude Code on your phone with NVIDIA full-power models via a lightweight Python proxy:
...
opsora-gateway && opsora-model balanced && opsora-claude
```

If `opsora-claude` fails with `/usr/bin/env: bad interpreter`, run:
