...
def execute_tool(name: str, args: dict) -> str:
    """Delegate to the main tool executor in opsora_v2 (imported lazily).

    Kept as a module-level indirection so sub-agent tool execution goes
    through one patchable name, and so opsora_v2 is only imported when a
    tool actually runs (avoids circular imports at module load time).
    """
    import opsora_v2
    return opsora_v2.execute_tool(name, args)
...
class SubagentOrchestrator:
    ...
    def _execute_tool(self, name: str, args: dict) -> str:
        """Delegate to the main tool executor via the module-level wrapper."""
        try:
